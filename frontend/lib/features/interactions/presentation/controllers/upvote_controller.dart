...
import 'package:frontend/features/interactions/data/upvote_repository.dart';
...
class UpvoteNotifier extends AsyncNotifier<UpvoteState> {
  UpvoteRepository get _repo => ref.read(upvoteRepositoryProvider);

  @override
  Future<UpvoteState> build() async {
    ref.clearOnLogout();
    final upvotes = await _repo.getUserUpvotes();
    return UpvoteState(
  ...
  Future<void> toggle(int serviceId) async {
    ...
    if (current.upvotes.containsKey(serviceId)) {
      ...
      await _repo.removeUpvote(upvoteId);
      ...
      final created = await _repo.upvoteService(serviceId);
...
// Convenience alias for callers that just need the report endpoint.
final reportProvider = Provider(
  (ref) => ref.read(upvoteRepositoryProvider),
);
